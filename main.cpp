...
int main(){
    ...
    Dot dot10(1100, 800), dot11(1100, 900), dot12(1200, 700);
    float x = 40.0;
    std::pair<float, float> speed1 = std::make_pair(x, x+2);
    std::pair<float, float> speed2 = std::make_pair(-1*x, -1*x);
    float mass = 200;

    Factory::createTriangle(dot1, dot2, dot3, speed2, mass, "test_texture.jpg");
    Factory::createTriangle(dot4, dot5, dot6, speed1, mass, "test_texture.jpg");
    Factory::createTriangle(dot7, dot8, dot9, std::make_pair(-1*x, x), 10*mass, "test_texture.jpg");
    Factory::createTriangle(dot10, dot11, dot12, std::make_pair(-1*x, -1*x), 2*mass, "test_texture.jpg");
    //createBorder();
